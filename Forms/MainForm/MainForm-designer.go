...
type MainForm struct {
	*goforms.Form
	...
	progressValue   int
	dateTimePicker1 *goforms.DateTimePicker
	comboBox1       *goforms.ComboBox
	treeView1       *goforms.TreeView
	button1         *goforms.Button
}

// NewMainForm mirrors `new MainForm()` — allocate, then InitializeComponent.
func NewMainForm() *MainForm {
	mf := &MainForm{Form: goforms.NewForm("GoForms Demo - Main Form", 1090, 532)}
...
func (mf *MainForm) initializeComponent() {
	...
	mf.chkSubscribe.SetBounds(10, 18, 220, 24)
	...
	mf.radioLight.SetChecked(false)
	...
	mf.dateTimePicker1 = goforms.NewDateTimePicker(time.Now())
	mf.dateTimePicker1.ValueChanged.Handle(mf.dateTimePicker1_ValueChanged)
	mf.dateTimePicker1.SetBounds(358, 286, 150, 30)
	mf.AddControl(mf.dateTimePicker1)
	mf.comboBox1 = goforms.NewComboBox()
	mf.comboBox1.SetBounds(340, 351, 150, 30)
	mf.AddControl(mf.comboBox1)
	mf.treeView1 = goforms.NewTreeView()
	mf.treeView1.SetBounds(515, 77, 200, 200)
	mf.AddControl(mf.treeView1)
	mf.button1 = goforms.NewButton("Button")
	mf.button1.Click.Handle(mf.button1_Click)
	mf.button1.SetBounds(800, 100, 90, 30)
	mf.AddControl(mf.button1)
}
