...
func (mf *MainForm) dateTimePicker1_ValueChanged(sender any, e goforms.EventArgs) {
}

func (mf *MainForm) button1_Click(sender any, e goforms.MouseEventArgs) {
	go func() {
		settingform.NewSettingForm().Form.ShowDialog()
		fyne.Do(func() {
			// if result == goforms.DialogOK {
			mf.lblGreeting.SetText("CLOSE SETTING FORM")
			// }
		})
	}()
	// goforms.Form = form
